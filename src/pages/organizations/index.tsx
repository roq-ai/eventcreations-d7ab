  ...
  const { data, error, isLoading, mutate } = useSWR<OrganizationInterface[]>(
    ...
    () =>
      getOrganizations({
        relations: [
          'user',
          'event_attendee.count',
          'event_sponsor.count',
          'event_team_member.count',
          'event_vendor.count',
        ],
      }),
  );
            ...
            <Table variant="simple">
              <Thead>
                <Tr>
                  ...
                  {hasAccess('event_attendee', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                    <Th>event_attendee</Th>
                  )}
                  {hasAccess('event_sponsor', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                    <Th>event_sponsor</Th>
                  )}
                  {hasAccess('event_team_member', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                    <Th>event_team_member</Th>
                  )}
              ...
              <Tbody>
                {data?.map((record) => (
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    {hasAccess('event_attendee', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>{record?._count?.event_attendee}</Td>
                    )}
                    {hasAccess('event_sponsor', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>{record?._count?.event_sponsor}</Td>
                    )}
                    {hasAccess('event_team_member', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>{record?._count?.event_team_member}</Td>
                    )}
                    {hasAccess('event_vendor', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
